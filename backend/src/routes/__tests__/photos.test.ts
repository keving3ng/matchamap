...
describe('Photo Upload Routes', () => {
  ...
  describe('uploadPhoto', () => {
    it('should upload photo successfully', async () => {
      ...
      expect(result.data.photo.imageUrl).toBe('https://photos.matchamap.app/test-key');
    });
    ...
    it('should return 400 when cafeId is missing', async () => {
      ...
      expect(response.status).toBe(HTTP_STATUS.BAD_REQUEST);
      expect(result.error).toBe('Cafe ID is required');
    });
  });
  ...
  describe('getUserPhotos', () => {
    it('should return user photos successfully', async () => {
      ...
      mockDbInstance.select().from().where().orderBy().limit.mockResolvedValue(mockPhotos);
  ...
  describe('deletePhoto', () => {
    ...
    it('should return 404 when photo not found', async () => {
      const mockDbInstance = await mockDb();
      mockDbInstance.delete().where().returning().get.mockResolvedValue(null);
  ...
  describe('getPhotosByCafe', () => {
    it('should return cafe photos successfully', async () => {
      ...
      const mockDbInstance = await mockDb();
      mockDbInstance.select().from().leftJoin().where().orderBy().limit.mockResolvedValue(mockPhotos);
    ...
    it('should return empty array for cafe with no photos', async () => {
      const mockDbInstance = await mockDb();
      mockDbInstance.select().from().leftJoin().where().orderBy().limit.mockResolvedValue([]);
